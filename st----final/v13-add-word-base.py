...
SMART: Timer only active when words detected, resets on same word count
"""
import numpy as np
import pyaudio
import threading
...
class WhisperStreamingTranscriberWithSpecials:
    def __init__(self, model_name="base.en", buffer_duration_seconds=5):
        ...
        # SMART: Word-based finalization timer (only active when words detected)
        ...
        self.last_new_word_time = None        # When we last detected new words OR same word count
        self.no_new_words_delay = 5.0         # Wait 5 seconds after no new words detected
        self.timer_is_active = False          # NEW: Track if timer is currently active
    ...
    def _commit_to_stable_buffer(self, stable_text, end_time):
        ...
        SMART: Reset word timer when new content is added
        """
        ...
        # SMART: Reset word timer when new stable content is added
        if self.timer_is_active:
            self.last_new_word_time = time.time()
            print(f"\033[93m[WORD TIMER RESET] New stable content added, resetting word timer\033[0m")
                ...
                if len(self.transcription_history) >= 3:
                    ...
                    if len(common_with_third) > 10 and common_with_third == self.confirmed_pattern:
                        ...
                        if end_time is not None:
                            # Commit to stable buffer (this will reset the word timer if active)
    ...
    def _reset_sentence_state(self, reason="Foreign language detection"):
        ...
        SMART: Only clear active components, preserve stable buffer and timer state
        """
        print(f"\n[RESET STATE] {reason}")
        print(f"[RESET STATE] Clearing ONLY active components, preserving stable buffer")
        
        # PRESERVE stable text buffer and timer state - DO NOT CLEAR THESE
        # self.stable_text_buffer = ""  # DON'T CLEAR - preserve good content
        # self.last_new_word_time = None  # DON'T CLEAR - preserve word timer
        # self.timer_is_active = False  # DON'T CLEAR - preserve timer state
        ...
        if self.timer_is_active and self.last_new_word_time:
            elapsed = time.time() - self.last_new_word_time
            print(f"[RESET STATE] Word timer continues: {elapsed:.1f}s since last activity")
        elif not self.timer_is_active:
            print(f"[RESET STATE] Word timer inactive (waiting for 0 → n transition)")
    ...
    def _has_new_words(self, current_transcription):
        """
        SMART: Check for word count changes and manage timer state accordingly
        Timer states:
        ...
        # Case 1: No words detected (0)
        if current_word_count == 0:
            if self.timer_is_active:
                # Deactivate timer when back to 0 words
                print(f"[WORD COUNT RESET] Word count back to 0 - deactivating timer")
                self.timer_is_active = False
                self.last_new_word_time = None
            self.last_word_count = 0
            return False
        
        # Case 2: First words detected (0 → n)
        if self.last_word_count == 0 and current_word_count > 0:
            print(f"[NEW WORDS DETECTED] Word count: {self.last_word_count} → {current_word_count}")
            print(f"\033[96m[WORD TIMER ACTIVATED] Starting 5s timer (0 → {current_word_count})\033[0m")
    ...
    def _should_finalize_no_new_words(self):
        """
        SMART: Check if we should finalize based on no new words detected
        Only active when timer_is_active = True
        Only returns True if 5 seconds have passed since last word activity
        """
        # Timer must be active to finalize
        if not self.timer_is_active or self.last_new_word_time is None:
            return False
        
        current_time = time.time()
        no_new_words_elapsed = current_time - self.last_new_word_time
        
        should_finalize = no_new_words_elapsed >= self.no_new_words_delay
    ...
    def _finalize_sentence(self, final_text=None):
        ...
        # SMART: Reset word timer and deactivate
        self.last_new_word_time = None
        self.timer_is_active = False
        print(f"\033[95m[WORD TIMER DEACTIVATED] Sentence finalized, timer deactivated\033[0m")
    ...
    def _process_audio(self):
        """Simplified audio processing with smart word detection timer"""
        while self.is_recording:
            try:
                chunk_count = 0
                start_time = time.time()
                
                # SMART: Check if we should finalize based on no new words timer (only when active)
                if self._should_finalize_no_new_words():
    ...
    def _process_sentence_segment(self, audio_buffer):
        """
        Process a sentence segment with enhanced dual buffer system and smart word timer
        """
        ...
        try:
            ...
            # SMART: Check word count changes and manage timer state
            if self._has_new_words(new_text):
                timer_status = "ACTIVE" if self.timer_is_active else "INACTIVE"
                print(f"[SMART TIMER] Timer is {timer_status}")
            ...
            if is_sentence_end:
                print(f"[SENTENCE END DETECTED] But relying only on smart word timer")
    ...
    def start_streaming(self):
        ...
        # SMART: Initialize word timer (inactive state)
        self.last_new_word_time = None
        self.timer_is_active = False
        ...
        try:
            ...
            print("Listening with SMART Word Detection Timer System...")
            print("- Text-only stable buffer (no audio buffer)")
            print("- SMART: Timer INACTIVE when word count = 0")
            print("- SMART: Timer ACTIVATES on 0 → n word transition")
            print("- SMART: Timer RESETS on n → n (same count) transitions")
            print("- Timer runs for 5 seconds after last word activity")
            print("- Stable buffer preserved during noise rejections")
            print("- Intelligent pattern detection with 3-way confirmation")
            print("- LLM sending when speaker stops talking (no word activity for 5s)")
            return True
        except Exception as e:
